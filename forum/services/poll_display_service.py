def attach_poll_data_to_posts(posts, serialized_posts):
    """Attach serialized poll/petition payloads to posts for template rendering."""
    ...
    petition_data_by_post_id = {
        serialized_post.get('id'): serialized_post.get('petition_data')
        for serialized_post in serialized_posts
    }
    for post in posts:
        post.petition_data = petition_data_by_post_id.get(post.id)
